...
const Sticker = ({ children, style }) => {
  ...
  return (
    <div ref={ref} className="sticker" style={style}
      onMouseMove={(e) => {
  ...
  return (
    <div className="page">
      <div className="hero">
        ...
        <div className="pot"><span style={{ fontSize: 26 }}>🏆</span>
          <div><div className="barlow muted" style={{ fontSize: 11 }}>Total pot</div>
            <div className="amt">{money(game.config.currency, pot)}</div></div>
        </div>
      </div>
      ...
      <div className="grid2" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))" }}>
        {rows.slice(0, 3).map((r, i) => (
          <Sticker key={r.p.id}>
            ...
            <div className="bebas gold" style={{ fontSize: 28, marginTop: 6 }}>{r.total} PTS</div>
          </Sticker>
        ))}
...
function LeaderboardPage({ game }) {
  ...
  return (
    <div className="page lb-wrap">
      ...
      {rows.map((r, i) => (
        <div key={r.p.id}>
          <div className="lb-row" onClick={() => setOpenRow(openRow === r.p.id ? null : r.p.id)}>
            <span className="rank">{i + 1}</span>
            <span>{r.p.avatar} {r.p.name} <span style={{ fontSize: 13 }}>{contention(r)}</span></span>
  ...
  return (
    <div className="page">
      ...
      <div className="panel" style={{ textAlign: "center" }}>
        ...
        <div className="bebas gold" style={{ fontSize: 52, textShadow: "0 0 20px rgba(240,201,58,.5)" }}>{money(cur, pot)}</div>
      </div>
      ...
      {PRIZES.map(([k, ic, lab, share, cond]) => {
        ...
        return (
          <div className="prize-row" key={k}>
            ...
            <span className="amt">{money(cur, pot * share)}</span>
          </div>
        );
      })}
    </div>
